...
export default function AdminPanel() {
  ...
  // NEW: Filter specifically for Drafted Players grouped by Team
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8 relative pb-20">
      ...
      <div className="md:col-span-3 space-y-8">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto max-h-[500px] border-t border-slate-100 pt-4 custom-scrollbar">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 overflow-hidden flex flex-col">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-black text-xl text-emerald-700">Team Rosters & Player List</h3>
            <span className="text-sm font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{draftedPlayers.length} Drafted</span>
          </div>
          
          <div className="overflow-y-auto max-h-[500px] border-t border-slate-100 pt-4 custom-scrollbar">
            {draftedPlayers.length === 0 ? (
              <p className="text-slate-500 font-medium text-center py-6 bg-slate-50 rounded-xl">No players have been auctioned to any team yet.</p>
            ) : (
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="text-slate-400 font-bold uppercase text-xs tracking-wider border-b border-slate-100">
                ...
                <tbody>
                  {draftedPlayers.map((u) => (
                    <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                      <td className="py-4 px-2 font-black text-indigo-700">{u.auctioned_to}</td>
                      <td className="py-4 px-2">
                        <span className="font-bold text-slate-800">{u.name}</span>
                        <span className={`ml-2 px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${u.gender === 'Female' ? 'bg-pink-100 text-pink-600' : 'bg-blue-100 text-blue-600'}`}>{u.gender}</span>
                      </td>
                      <td className="py-4 px-2 font-semibold text-slate-600">{u.phone}</td>
                      <td className="py-4 px-2 text-right font-bold text-emerald-600">₹{u.auction_price}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {editingUser && (
        // ... (Keep your exact existing modal code here, no changes needed to the modal itself!)
